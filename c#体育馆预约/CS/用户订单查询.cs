...
#region Default Instance
		...
		public void btnSelectOrders_Click_1(object sender, EventArgs e)
		{
			data = DateTime.Parse(dtpTime.Value.Date.ToShortDateString());
			...
			if (cbmGym.Text == "")
			{
				...
				ads.Fill(ds, "Stable");
				this.gvwOrders.DataSource = ds.Tables[0];
			}
			//地点不空
			if (cbmGym.Text != "")
			{
				com.CommandText = "select Vname,Vno,Stime,Etime,LTime,Fee,Orderno,Remark from Orders where ID='" + ID + "'and convert(varchar(10),Stime,120) = '" + System.Convert.ToString(data) + "'and Vname = '" + cbmGym.Text + "'";
				com.Connection = con;
				ads.SelectCommand = com;
				ads.Fill(ds, "Stable");
				this.gvwOrders.DataSource = ds.Tables[0];
		...
		public void btnSelectallOrders_Click(object sender, EventArgs e)
		{
			ds.Clear();
